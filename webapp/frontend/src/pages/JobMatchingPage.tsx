...
// The overlay can show any matched part, or the whole mold base.
const BASE_TARGET = "__BASE__" as const;
type OverlayTarget = ComponentKind | typeof BASE_TARGET;
...
export default function JobMatchingPage() {
  ...
  const [overlayKind, setOverlayKind] = useState<OverlayTarget | null>(null);
    ...
    api
      ...
      .then((detail) =>
        buildJobSignature(
          ...
          detail.models,
          massByKindFromParts(detail.parts),
          maxDimByKindFromParts(detail.parts)
        )
      )
    ...
    try {
      ...
      for (const summary of candidates) {
        try {
          ...
          const sig = await buildJobSignature(
            summary.job_id,
  ...
  // Both jobs need a whole-base STL before the full-base overlay is offerable.
  const canOverlayBase = Boolean(currentSignature?.base && selectedSignature?.base);

  const chooseMatch = (jobId: string) => {
    setSelectedMatchId(jobId);
    const match = jobMatches?.find((m) => m.jobId === jobId);
    const candidate = candidateSignatures.get(jobId);
    // Default to the whole base when both sides have one -- seeing the two
    // complete bases stacked is the fastest read on whether it's the same tool.
    if (currentSignature?.base && candidate?.base) setOverlayKind(BASE_TARGET);
    else setOverlayKind(match?.components[0]?.kind ?? null);
  };

  const overlayTargetFor = (sig: JobSignature | null) => {
    if (!sig || !overlayKind) return undefined;
    if (overlayKind === BASE_TARGET) return sig.base?.model.url;
    return sig.components.get(overlayKind)?.model.url;
  };

  const overlayUrlA = overlayTargetFor(currentSignature);
  const overlayUrlB = overlayTargetFor(selectedSignature);
  const overlayComponentScore =
    overlayKind === BASE_TARGET
      ? null
      : selectedMatch?.components.find((c) => c.kind === overlayKind) || null;
              ...
              <Card className="p-4">
                ...
                <div className="mb-4 flex flex-wrap gap-2">
                  {canOverlayBase && (
                    <button
                      type="button"
                      onClick={() => setOverlayKind(BASE_TARGET)}
                    ...
                    >
                      Full base
                      <Badge tone="brand">{selectedMatch.score.toFixed(0)}%</Badge>
                    </button>
                  )}
                ...
                <div className="h-[420px]">
                  {overlayUrlA && overlayUrlB ? (
                    ...
                    <div className="flex h-full items-center justify-center text-xs text-ink-500">
                      Pick a part or the full base above
                    </div>
                  )}
                </div>
                ...
                {overlayKind === BASE_TARGET ? (
                  <div className="mt-4 text-xs text-ink-400">
                    Whole mold base, both jobs in the same corrected CMS frame · overall match{" "}
                    {selectedMatch.score.toFixed(1)}% across {selectedMatch.matchedComponents}/
                    {SIGNATURE_KINDS.length} parts
                  </div>
                ) : (
