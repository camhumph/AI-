...
Private Function StdFullPlateNameFromGeometry(ByVal pos As Long, ByVal nFull As Long, ByVal cadIdx As Long) As String
    ...
    Else
        StdFullPlateNameFromGeometry = StdFullPlateName(pos, nFull)
    End If
End Function
...
Private Sub BuildStdFromGeometry()
    ...
    ' Direction: the end nearer the ejector plates is the bottom. If there are
    ' no ejector plates in the model, use end-plate hints and standard clamp
    ' thickness behavior so unnamed standard stacks do not flip top/bottom.
    ...
    For i = 1 To nFull
        AddStdPlateFromCad fullIdx(i), StdFullPlateNameFromGeometry(i, nFull, fullIdx(i))
    Next i
...
Private Function TryClassifyStandardCadPurchased(ByVal idx As Long, _
    ...
    If Left(UCase(partNo), 2) = "LP" Then
        desc = "Leader Pin"
        vendor = "PCS"
    ...
    If Left(UCase(partNo), 3) = "GEB" Then
...
On Error GoTo eh
    ...
    f = FreeFile
    Open destPath For Output As #f
    Print #f, "Index,CurrentName,GeometryRole,SuggestedPCSName,Confidence,Reason,Thickness,Width,Length,CenterX,CenterY,CenterZ"
...
Private Function GeometryRoleForCadIndex(ByVal idx As Long, ByVal isStandardBase As Boolean) As String
    ...
    If IsRoundBarLike(idx) Then
        ...
        If parts(idx).Length / IIf(dia <= 0#, 1#, dia) >= 3.5 Then
            If dia >= 1.5 Then
                GeometryRoleForCadIndex = "PILLAR / LEADER PIN"
            Else
                GeometryRoleForCadIndex = "EJECTOR LEADER PIN / SMALL LEADER PIN"
            End If
        Else
            GeometryRoleForCadIndex = "ROUND BUSHING / SLEEVE"
        End If
        Exit Function
    End If
...
Private Function SuggestedPcsNameForCadIndex(ByVal idx As Long, ByVal role As String, _
    ...
    Select Case NormalizeKey(role)
        Case "TCP", "BCP", "IDHOLDER", "ODHOLDER", "IDPOT", "ODPOT"
            ...
            Exit Function
        Case "STANDARDPLATE/RAIL", "STANDARDPLATERAIL"
            SuggestedPcsNameForCadIndex = CleanFileName(IIf(JobBaseName <> "", JobBaseName, CurrentJobNumber) & "_PLATE-LIKE")
            confidence = "LOW"
            reason = "Plate-like geometry; exact standard-plate role depends on stack order and BOM."
            Exit Function
    End Select
