...
Private Function HoleSigCandidates(ByRef outIdx() As Long) As Long
    ...
    For i = 1 To PartCount
        If parts(i).Thickness >= HOLE_SIG_MIN_THICKNESS Then
            fp = parts(i).Width * parts(i).Length
            If fp >= maxFoot * HOLE_SIG_MIN_FOOTPRINT_FRAC Then
                n = n + 1
                outIdx(n) = i
            End If
        End If
...
On Error GoTo eh
    ...
    AddStdPlate "Rails", parts(railIdx(1)).Thickness, parts(railIdx(1)).Width, _
                parts(railIdx(1)).Length, n, "", railIdx(1)
...
Private Sub AddStdPlateFromCad(ByVal idx As Long, ByVal nm As String)
    ...
    ' A DOUBLED BOUNDING BOX IS A DOUBLED STEEL BILL.
    '
    ' On C18599 the A plate measured T=11.910 while its own name says 5-7/8:
    '
    '     A PLATE(17-78X35-12X5-78)   named 5.875   measured 11.910   ratio 2.03
    '
    ' Width and length matched to four decimals, so the box is right in plane and
    ' wrong through thickness -- the signature of two coincident instances of the
    ' same plate, where GetBox returns the combined solid. The B plate on the same
    ' job measured 3.387 against a named 3.375, so this is not a units or
    ' orientation problem; it is specific to that one component.
    '
    ' Left alone, the most expensive plate in the base gets quoted at twice its
    ' thickness.
    '
    ' Correct ONLY on a near-exact integer multiple (2x, 3x), because that is what
    ' duplicate bodies look like. Any other disagreement is something else and the
    ' measurement is kept -- with a warning, because a silent guess about steel is
    ' worse than a loud question.
    Dim namedT As Double, ratio As Double, mult As Double
    ...
    If namedT > 0.01 And t > 0.01 Then
        ratio = t / namedT
        mult = CDbl(CLng(ratio))
        If Abs(ratio - 1) > 0.03 Then
            If mult >= 2 And Abs(ratio - mult) <= 0.05 Then
                LogLine "PLATE THICKNESS CORRECTED: " & nm & _
                        " measured T=" & Format(t, "0.000") & " is " & Format(mult, "0") & _
                        "x the thickness its own name states (" & Format(namedT, "0.000") & _
                        "). That is duplicate coincident bodies, not a thicker plate -- " & _
                        "using the named size so the steel is not quoted " & Format(mult, "0") & "x."
                t = namedT
            Else
                LogLine "WARNING: PLATE THICKNESS DISAGREES: " & nm & _
                        " measured T=" & Format(t, "0.000") & " but its name states " & _
                        Format(namedT, "0.000") & " (ratio " & Format(ratio, "0.00") & _
                        "). Not a clean multiple, so the MEASURED value is kept -- check " & _
                        "this plate on the steel sheet before cutting."
            End If
        End If
    End If
...
On Error GoTo ErrHandler
    ...
    If nRailCand > 0 Then
        ...
        AddStdPlate "Rails", _
                    parts(railCand(1)).Thickness, _
                    parts(railCand(1)).Width, _
                    parts(railCand(1)).Length, _
                    nRailCand, _
                    "", _
                    railCand(1)
...
On Error GoTo eh
    ...
    If InStr(uLeaf, "STRIPPER-PLATE") > 0 Or InStr(uLeaf, "STRIPPER_PLATE") > 0 Then
    ...
    ' uDash folds "_" to "-" so each token below is written once instead of
    ' four times. The shop's STEP exports use both separators interchangeably
    ' and the earlier hand-listed variants had gaps: X-1_PLATE and PLATE_X1
    ' matched nothing at all.
    Dim uDash As String
    uDash = Replace(uLeaf, "_", "-")
    ...
    If InStr(uDash, "CAVITY-PLATE") > 0 Or InStr(uDash, "CAVITY-RETAINER") > 0 Then
        HardStandardRoleFromCadName = "A Plate"
        Exit Function
    End If

    If InStr(uDash, "CORE-PLATE") > 0 Or InStr(uDash, "CORE-RETAINER") > 0 Then
        HardStandardRoleFromCadName = "B Plate"
        Exit Function
    End If

    ' Deliberately NOT handled here: a bare "CLAMP-PLATE" with no TOP/BOTTOM
    ' qualifier. C18503's BOM calls it the BOTTOM clamp plate, but that is one
    ' customer's habit, not a rule -- geometry names the outermost plate
    ' correctly from stack position, so leave it alone rather than guess.
...
On Error Resume Next
    ...
    ' === CMS PATCH FINAL NO STANDARD COMPONENT CAPTURE ===
    If gJobIsStandardBase Then
        LogLine "STANDARD job: skipping CAD purchased-component capture/pricing. AI/geometry will classify steel only."
        Exit Sub
    End If
...
Private Sub ComputePurchasedQuote()
    ...
    ' === CMS PATCH FINAL NO STANDARD COMPONENT PRICING ===
    If gJobIsStandardBase Then
        PpCount = 0
        ReDim PpDesc(1 To 1)
        ReDim PpQty(1 To 1)
        ...
        LogLine "STANDARD job: purchased component pricing disabled. No component prices will be written to the quote sheet."
        Exit Sub
    End If
    If PpCount < 1 Then
